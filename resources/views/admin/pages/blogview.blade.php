...
@section('content')
<?php use \App\Http\Controllers\BlogComment; ?>
            ...
            <div class="row">
                ...
                @if (count($data[0]['blogdata']) > 0)
                    ...
                    <div class="box box-widget">
                        ...
                        {!! $blog->message !!}
                        <button type="button" class="btn btn-default btn-xs" onclick="socialShare('eventshare', 'blog', '{{ $blog->subject }}', '{{ $blog->id }}')"><i class="fa fa-share"></i> Share</button>
                        <a type="button" class="btn btn-success btn-xs" href='/admin/blogedit/{{ $blog->id }}'><i class="fa fa-edit"></i> Edit</a>
                        <button type="button" class="btn btn-danger btn-xs" onclick="deletePost('{{ $blog->id }}', 'blog')"><i class="fa fa-trash"></i> Delete</button>

                        <button type="button" class="btn btn-primary btn-xs" onclick="likeBlog('{{ $blog->id }}')"><i class="fa fa-thumbs-o-up"></i> Like</button>
                        <span class="pull-right text-muted"><b id="likey{{ $blog->id }}">{{ $blog->likes }}</b> likes - <b>@if($commentcount = \App\BlogComment::where('blog_id', $blog->id)->count()) {{ $commentcount }} @else 0 @endif</b> comments</span>
                        </div>
                        <!-- /.box-body -->
                        <div class="box-footer box-comments" style="height: 300px; overflow-y: auto;">
                            @if($comment = \App\BlogComment::where('blog_id', $blog->id)->orderBy('created_at', 'DESC')->get())
                ...
                @else
                    <div class="col-md-12">
                        <center>No blog post yet</center>
                    </div>
                @endif
